        ...
        <el-form :inline="true" :size="option.size" :model="query">
          <template>
            <el-form-item label="字段">
              <el-input v-model="query.type" placeholder="请输入商品类型:normal=实体商品,virtual=虚拟商品"></el-input>
            </el-form-item>
            <el-form-item label="字段">
              <el-input v-model="query.title" placeholder="请输入标题"></el-input>
            </el-form-item>
            <el-form-item label="字段">
              <el-input v-model="query.status" placeholder="请输入商品状态: 0:hidden=隐藏商品,1:up=上架,2:down=下架"></el-input>
            </el-form-item>
          </template>
          <!-- 查询按钮 -->
          <el-form-item>
      ...
      <el-dialog :title="title" :visible.sync="box" width="50%" :before-close="beforeClose" append-to-body>
        <el-form :disabled="view" :size="option.size" ref="form" :model="form" label-width="80px">
          ...
          <el-form-item label="发货方式:express=物流快递,selfetch=用户自提,store=商家配送,autosend=自动发货" prop="dispatchType">
            <el-input v-model="form.dispatchType" placeholder="请输入发货方式:express=物流快递,selfetch=用户自提,store=商家配送,autosend=自动发货"/>
          </el-form-item>
